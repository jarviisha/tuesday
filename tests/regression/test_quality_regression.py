...
@pytest.mark.anyio
async def test_regression_suite_preserves_quality_invariants() -> None:
...
@pytest.mark.anyio
async def test_regression_suite_preserves_reindex_and_filter_semantics() -> None:
    transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
        first_index = await client.post("/documents/index", json=REFUND_DOCUMENT)
        second_index = await client.post("/documents/index", json=REFUND_DOCUMENT)
