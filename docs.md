...
- **Chunking**: theo **token** qua `LlamaIndex SentenceSplitter`; `chunk_overlap < chunk_size` (DL-006 superseded bởi DL-035).
...
| `DL-006` | `superseded` | ~~Chunking MVP dùng cấu hình theo ký tự với `chunk_size = 1000`, `chunk_overlap = 150` trong biên spec~~ → superseded bởi DL-035 (token-based chunking qua LlamaIndex `SentenceSplitter`) | Tránh coupling vào tokenizer hoặc model cụ thể quá sớm | `config`, `chunking`, use case ingestion, tests |
...
| `DL-035` | `accepted` | Migration `rag/infrastructure/` sang LlamaIndex end-to-end (spec 002): `LlamaIndexNodeParser` (token-based `SentenceSplitter`, supersedes DL-006), `LlamaIndexQdrantAdapter` (Qdrant via `QdrantVectorStore.add/delete/query`, collection prefix `tuesday_v2__`), `LlamaIndexEmbeddingAdapter` + `LlamaIndexLLMAdapter` (wrapping LlamaIndex vendor connectors, xóa `http_client.py`); demo providers extend `BaseEmbedding`/`CustomLLM`; `VectorStoreIndex` bị loại bỏ để tránh `Settings.embed_model` initialization bug | Piecemeal adoption đã bị reject; full swap hạ tầng trong biên `infrastructure/` giúp loại bỏ custom HTTP code, tận dụng LlamaIndex vendor connectors, và đặt nền cho reranking/hybrid search tương lai mà không đổi domain Protocols hay public API | `specs/002-llamaindex-rag-migration/`, `infrastructure/`, `runtime/container.py`, `runtime/config.py`, test suites |
