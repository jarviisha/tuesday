...
| `DL-019` | `accepted` | Sau Phase 3, repo **chấp nhận full migration** cấu trúc `src/tuesday_rag` sang layout capability-based để scale ownership và mở rộng feature dễ hơn; migration này phải đi trước hoặc cùng lúc với feature expansion đầu tiên | Team đã có baseline vận hành, hardening và benchmark đủ để đo regress, nên chi phí migration lớn nay được chấp nhận để đổi lấy khả năng scale cấu trúc dài hạn | feature expansion planning, architecture migration, benchmark/regression recheck, docs post-MVP |
